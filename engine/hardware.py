...
def detect_hardware() -> Dict[str, Any]:
    ...
    if cuda_available:
        ...
        # Use int8 for CPU on Linux/Windows for performance
        # Keep float32 on macOS to prevent instability
        if platform.system() == "Darwin":
            hardware_info["compute_type"] = "float32"
            print(f"⚠ No CUDA GPU detected. Using CPU with float32 (macOS stability)")
        else:
            hardware_info["compute_type"] = "int8"
            print(f"⚠ No CUDA GPU detected. Using CPU with int8 (optimized)")
